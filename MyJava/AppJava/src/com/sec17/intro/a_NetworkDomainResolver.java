...
public class a_NetworkDomainResolver {
	...
	public static void main(String[] args) {
		...
		try {
			// 1. 문자열을 URL 객체로 변환
			URL url = URI.create(urlString).toURL(); //url로 만들어줌
			...
			// 2. 도메인 → IP 주소 변환
			InetAddress address = InetAddress.getByName(url.getHost()); //getByName : 호스트 하나
			...
			Arrays.stream(all).forEach(ip -> System.out.println(" - " + ip.getHostAddress())); //all이 배열이라 forEach로 출력
