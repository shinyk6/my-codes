...
user:	사용자 계정 정보 (이름, 호스트, 비밀번호, 글로벌 권한 등)						-> 사용자 계정정보를 가지고 있는 테이블들
...
FLUSH PRIVILEGES : 변경된 권한을 즉시 반영     -- 이것을 안주면 다음에 재접속 할 때 권한이 생김
 */
 -- 사용자 권한 보기
SELECT host, user FROM mysql.user;

-- 사용자별 권한 확인
SHOW GRANTS FOR 'uk60'@'%';
...
FLUSH PRIVILEGES;
-- cmd에서 확인
-- mysql -u mysample07 -p -h localhost
-- 패스워드 test
-- select current_user();
-- use myworld;
