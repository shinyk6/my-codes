...
USE MY_EXAM;  ##DB만들어서 작성
CREATE TABLE t (
b BIT(8)
); # 8BIT를 사용하겠다 / 부울 값(TRUE,FALSE) /플래그 

#MYSQL 
-- Insert into 테이블명 values(값); //데이터 순서가 타입에 맞아야함
	-- ex) Insert into 테이블명 values(100, 'a', null);
    -- ex) Insert into 테이블명 values(100, null, 'a');   number, varchar, varchar
-- Insert into 테이블명(컬럼명) values(값);  //1대1로 줄 수 있음
	-- ex) Insert into 테이블명(이름) values('길동');
...
-- Q5  CAST함수를 이용해보자.   정수로 변환해보자.  명시 캐스팅
HELP CAST;
...
CREATE TABLE t1 (
  ...
  -- UUID_TO_BIN(UUID())   VER. 8.0에 도입→ 인덱스 성능향상 효과 16진수 바이너리 데이터 사용
  -- UUID()는 보통 32개 자리를 사용 -> binary(16)자리로 변경해서 사용
  i INT         DEFAULT 0,        --  DEFAULT : 기본값
  c VARCHAR(10) DEFAULT 'ABCD',
  -- expression defaults ->
  f FLOAT       DEFAULT (RAND() * RAND()),
  b BINARY(16)  DEFAULT (UUID_TO_BIN(UUID())), -- uuid() mySql에서 고유 id생성
...
-- JSON_ARRAYGG() : group by 한 후 집계결과를 json배열로 리턴
