...
LAST_VALUE(): 	그룹 내 마지막 값 -> 프렘임 지정(ROWS) 필수!!!
NTH_VALUE()	: N번째 값 가져오기

CUME_DIST()	: 누적 비율 계산   (누적 / 전체 행) → 상위 %를 나타낼 때 사용. 첫번째 값은 일정 비율값
							-- (현재 행보다 같거나 작은값의 개수 / 전체 행)
...
-- Q1 급여가 높은 순서대로 정렬하고 각 사원의 {2번째 이전 사원 급여} = LAG(SAL,2)를 리턴 해보자
-- 없으면 0을 리턴해라 
    SELECT ENAME, SAL, LAG(SAL , 2, 0)  OVER (ORDER BY SAL DESC)  AS res
 FROM EMP;

-- Q1-2 부서별로 사원 급여를 확인해보자. 각 사원의 '이전사원급여' 를 출력하자.
-- 없으면 0으로 리턴
     SELECT ENAME, SAL, LAG(SAL , 1, 0)  OVER (partition by deptno ORDER BY sal desc)  AS 이전사원급여
 FROM EMP;
...
FROM EMP;
-- FIRST_VALUE(SAL) OVER -> 정렬된 결과에서 가장 첫번째 급여값을 리턴
/*
    LAST_VALUE(SAL) OVER (PARTITION BY DEPTNO
						  ORDER BY SAL DESC 
        ROWS BETWEEN UNBOUNDED PRECEDING AND UNBOUNDED FOLLOWING) - 처음부터 끝까지between으로
	마지막 행의 급여값을 리턴 - 항상 현재프레임의 마지막을 현재 행이라고 생각
    결론 LAST_VALUE(SAL) + ROWS 함께 사용 . 프레임 속성값을 지정하지않으면 "현재행"을 리턴한다.
*/
...
SELECT   DEPTNO, ENAME, SAL, 
         LAST_VALUE(SAL) OVER (
         PARTITION BY DEPTNO 
         ORDER BY SAL DESC  -- 그룹별 orderby
        -- ROWS BETWEEN 1 PRECEDING AND CURRENT ROW   # 마지막 값을 현재 행의 값으로 리턴  --current row를 주던 안주던
    ) AS RES
FROM EMP
...
-- Q8)  사원테이블에서 봉급의 백분율를 계산해 보자.   - 상대적 순위  PERCENT_RANK()
-- 단, 소수 두 자리까지 표시해보자.
-- PERCENT_RANK		= (순위 - 1)/(전체 행의 수 -1)    => % 전체행에서 순위 나누기
-- (rank - 1)/(rows - 1)
   SELECT SAL ,
   CONCAT(Round(PERCENT_RANK() OVER (ORDER BY SAL ),2), '%') AS RES
   FROM EMP;
-- format() : 리턴값 String, ROUND() : 리턴값 NUMBER
...
--       3월 26일 	---
-- CASE 2 : 추출함수 사용 EXTRACR(unit FROM date)    EXTRACT(날짜속성 FROM date컬럼명)

-- EXTRACR(unit FROM date)    EXTRACT(날짜속성 FROM date컬럼명)
...
-- CASE 3 : 추출함수 사용 EXTRACR(unit FROM date)    EXTRACT(날짜속성 FROM date컬럼명)사용하고 별칭사용
SELECT   EXTRACT(YEAR FROM HIREDATE)  AS  년도 , 
          COUNT(*) AS EMP_COUNT
FROM   EMP
GROUP BY   년도    ##############컬럼 별칭사용  
...
-- Q14)  사원테이블에서 부서 번호와 각 부서 (DEPTNO) 직원의 평균 재직 기간(년)을 출력 하자.  
 ...
 -- 표본분산 	 VAR_SAMP(expr) [over_clause] : 윈도함수	N-1로 나누는 방식으로 계산된다.
...
-- CASE 2. 윈도우함수를 사용하여 부서별로 표본분산을 출력하자.
   SELECT DEPTNO , 
 ...
 --  STDDEV(EXPR)  :  표준 편차가 높을 수록 급여의 범위가 넓다.  STDDEV_POP()
 -- 사원의 각 부서별 봉급의 표준 편차 계산해 보자.  
 -- 사원의 각 부서별 표준편차 계산해보자.                 
 SELECT DEPTNO, 
 ...
 그룹된 값을 (,)로 연결해서 하나의 문자열을 만들어 준다.
GROUP_CONCAT([DISTINCT] expr [,expr ...]
             [ORDER BY {unsigned_integer | col_name | expr}
                 [ASC | DESC] [,col_name ...]]
             [SEPARATOR str_val])  : STRING 리턴
