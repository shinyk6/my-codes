...
-- 테이블의 구조를 확인해보자	22번 부터 CITY테이블 문제는 스킵
desc city;
desc country;
...
USE MY_EMP;
-- Q30)  특정 달에 입사한 직원을 조회 해보자. **꼭 풀어보기
-- 1981년도 6월에 입사한 직원들의 사원번호, 사원의 이름, 입사일  
SELECT EMPNO, ENAME, HIREDATE
FROM EMP
WHERE  YEAR(HIREDATE)  = 1981  AND MONTH(HIREDATE )  =6;

-- Q31) 입사 후  근무 기간이 5년 이상인 직원들의 사원번호, 사원의 이름, 입사일을 출력해보자.  **꼭 풀어보기
...
3. 날짜 계산
	DATE_ADD(date, INTERVAL value unit): 날짜에 지정된 시간 간격을 추가. -상수 계산해서 요일 증가
	DATE_SUB(date, INTERVAL value unit): 날짜에서 지정된 시간 간격 리턴. -상수 계산해서 요일 감소
	DATEDIFF(date1, date2): 두 날짜 간의 차이를 {일수}로 리턴. 
4. 날짜 형식 변환
