...
*/
DROP TABLE t1;
######################## 인덱스 없이 RANGE 파티션 선언 후 데이터 입력-> 파티션 확인-> COST비용 확인-> 파티션의 목록 확인
-- 인덱스 추가 -> 파티션 확인, COST비용 확인-> 파티션의 목록 확인
...
PARTITION BY RANGE (year_col) ( -- 데이터의 범위로 분할하겠다.
...
EXPLAIN Analyze SELECT  * FROM T1 WHERE year_col < 1990; -- COST확인
...
-- 9-1. 범위에 없는 데이터를 추가해보자.
INSERT INTO T1 VALUES (6, 2025); -- X
INSERT INTO T1 VALUES (7, 1888); -- O
INSERT INTO T1 VALUES (8, NULL); -- O
...
-- 파티션 범위에 없어 Error Code: 1526. Table has no partition for value 2025
INSERT INTO T2 VALUES (6, 2025); -- 파티션 나눈 것에서 범위 벗어남 -> 리스트 목록에 있는 것들만 들어감
INSERT INTO T2 VALUES (7, 1888); -- 범위 벗어남 
...
PARTITIONS 4; -- 파티션의 NAME이 자동으로 생김
...
delete from T3;
            #######
CALL T3_INSERT(10000); -- 프로시저 작성 후 데이터 입력
 -- year_col 에 INSERT
...
-- 11-5-1) 전체 1998을 찾아서 전체 출력하는 코드
SELECT  *
...
EXPLAIN SELECT * FROM t4 PARTITION (p0); -- rows 1  확인   -> 최소 1개정도는 있을 거라는 예상통계 (Key값이 걸려있기때문에 가지고 있기 때문에)
